...
class Button:
    ...
    def __init__(self, x: int, y: int, width: int, height: int, 
                 ...
                 theme: ColorTheme = None, is_toggle: bool = False):
        ...
        self.is_toggle = is_toggle  # 是否为切换按钮
        self.is_active = False  # 切换状态（仅对 toggle 按钮有效）
...
class PetriVisualizer:
    ...
    def __init__(self, env: Env_PN, model_path: Optional[str] = None):
        ...
        pygame.init()
        pygame.display.set_caption("Wafer Processing Control Console")
        
        # 创建窗口图标（晶圆图案）
        icon = pygame.Surface((32, 32), pygame.SRCALPHA)
        icon.fill((0, 0, 0, 0))  # 透明背景
        # 外圈 - 绿色
        pygame.draw.circle(icon, (0, 255, 65), (16, 16), 14)
        # 中圈 - 深色
        pygame.draw.circle(icon, (40, 48, 60), (16, 16), 10)
        # 内圈 - 绿色小点
        pygame.draw.circle(icon, (0, 255, 65), (16, 16), 4)
        ...
        # 自动执行状态
        self.auto_model_running = False
        self.last_auto_step_time = 0.0
        self.AUTO_STEP_INTERVAL = 1  # 1秒间隔
        
        # 窗口缩放状态
        self.scale_factor = 1.0
        self.base_width = 1350
    ...
    def _setup_shortcuts(self):
        ...
        self.shortcuts = {
            ...
            pygame.K_r: -1,  # Random
            pygame.K_m: -3,  # Model(one step)
            pygame.K_a: -4,  # Model(auto)
    ...
    def load_model(self, model_path: str):
        ...
        try:
            ...
            # 维度不匹配时（如网络结构变化后使用旧模型）
            print(f"警告: 模型维度不匹配，无法加载 (当前 obs={n_obs}, actions={n_actions})")
            print(f"       需要重新训练模型。Model 按钮已禁用。")
            self.policy = None
            return False
    ...
    def _setup_layout(self):
        ...
        self.chamber_config = {
            ...
            # TM3 区域（现在活跃）
            "LLC": {"source": "s2", "active": True, "proc_time": 0, "robot": ["TM2", "TM3"]},
            "LLD": {"source": "s4", "active": True, "proc_time": 70, "robot": ["TM2", "TM3"]},
        ...
        # 运输库所（包括 TM2 和 TM3 区域）
        self.transports = ["d_s1", "d_s2", "d_s3", "d_s4", "d_s5", "d_LP_done"]
        ...
        grid_center_y = self.WINDOW_HEIGHT // 2 + 50  # 稍微下移一点
    ...
    def _setup_buttons(self):
        ...
        self.model_button = Button(panel_x, y, button_width, button_height,
                                   "Model(one step)", -3, "M", self.theme)
        self.buttons.append(self.model_button)
        self.button_groups["control"]["buttons"].append(self.model_button)
        y += button_height + button_gap
        
        # Model(auto) 按钮 - 切换按钮
        self.auto_model_button = Button(panel_x, y, button_width, button_height,
                                        "Model(auto)", -4, "A", self.theme, is_toggle=True)
        self.buttons.append(self.auto_model_button)
        self.button_groups["control"]["buttons"].append(self.auto_model_button)
        
        y += button_height + 20
        
        # Scale 按钮组
        scale_label_surf = self.font_tiny.render("SCALE", True, self.theme.text_secondary)
        # 三个小按钮横向排列
        scale_button_width = (button_width - 8) // 3
        scale_button_height = 28
        
        self.scale_buttons = []
        scale_values = [1.0, 2.0, 5.0]
        scale_labels = ["1x", "2x", "5x"]
        
        for i, (scale_val, label) in enumerate(zip(scale_values, scale_labels)):
            btn_x = panel_x + i * (scale_button_width + 4)
            scale_btn = Button(btn_x, y, scale_button_width, scale_button_height,
                              label, -10 - i, "", self.theme)
            scale_btn.enabled = True
            scale_btn.scale_value = scale_val  # 存储缩放值
            self.scale_buttons.append(scale_btn)
        
        y += scale_button_height + 10
    ...
    def _update_buttons(self, action_mask: np.ndarray):
        ...
        for btn in self.buttons:
            if btn.action_id >= 0 and btn.action_id < len(action_mask):
                ...
                btn.enabled = True
            elif btn.action_id == -3:  # Model(one step)
    ...
    def _draw_left_panel(self):
        ...
        total_wafers = self.net.n_wafer  # 使用配置的总晶圆数
        
        progress_label = self.font_small.render(f"PROGRESS: {done_wafers}/{total_wafers}", True, self.theme.text_secondary)
        ...
        progress = done_wafers / max(1, total_wafers)  # 使用正确的总数计算进度
        if progress > 0:
        ...
        # ===== 统计信息区域 =====
        y = self._draw_statistics_section(panel_x, panel_width, y)
    ...
    def _draw_statistics_section(self, panel_x: int, panel_width: int, y: int) -> int:
        """
        绘制统计信息区域 - 晶圆滞留时间统计
        ...
        """
        # 获取统计数据
        stats = self.net.calc_wafer_statistics()
        
        # ===== 标题 =====
        section_title = self.font_medium.render("STATISTICS", True, self.theme.accent)
        self.screen.blit(section_title, (panel_x + 12, y))
        y += 26
        
        # ===== 系统滞留时间 =====
        # 平均系统时间
        sys_avg = stats.get("system_avg", 0)
        sys_max = stats.get("system_max", 0)
        sys_diff = stats.get("system_diff", 0)
        ...
        sys_label = self.font_tiny.render("SYS TIME", True, self.theme.text_muted)
        self.screen.blit(sys_label, (panel_x + 12, y + 5))
        
        sys_value = f"avg:{sys_avg:.0f}s|max:{sys_max}s|diff:{sys_diff:.0f}s"
        sys_color = self.theme.accent if sys_avg > 0 else self.theme.text_secondary
        sys_surf = self.font_mono.render(sys_value, True, sys_color)
        sys_rect = sys_surf.get_rect(right=panel_x + panel_width - 12, centery=y + card_height // 2)
        self.screen.blit(sys_surf, sys_rect)
        y += card_height + 4
        
        # 完成/进行中数量
        count_rect = pygame.Rect(panel_x + 8, y, panel_width - 16, card_height)
        pygame.draw.rect(self.screen, self.theme.bg_surface, count_rect, border_radius=4)
        
        count_label = self.font_tiny.render("WAFERS", True, self.theme.text_muted)
        self.screen.blit(count_label, (panel_x + 12, y + 5))

        output = (completed/self.net.time)*3600 if self.net.time > 0 else 0
        count_value = f"output:{output:.1f}/h |done:{completed}"
        count_surf = self.font_mono.render(count_value, True, self.theme.text_secondary)
        count_rect = count_surf.get_rect(right=panel_x + panel_width - 12, centery=y + card_height // 2)
        self.screen.blit(count_surf, count_rect)
        y += card_height + 6
        
        # ===== 腔室滞留时间 =====
        chamber_label = self.font_tiny.render("CHAMBERS", True, self.theme.text_muted)
        self.screen.blit(chamber_label, (panel_x + 12, y))
        y += 16
        
        chambers = stats.get("chambers", {})
        # 只显示有驻留约束的腔室：PM7/8, PM1/2, PM9/10
        constrained_chambers = ["PM7/8", "PM1/2", "PM9/10"]
        
        for chamber_name in constrained_chambers:
            chamber_data = chambers.get(chamber_name, {"avg": 0, "max": 0, "count": 0})
            avg_time = chamber_data.get("avg", 0)
            max_time = chamber_data.get("max", 0)
            count = chamber_data.get("count", 0)
            
            # 获取该腔室的加工时间阈值
            proc_time = 70  # 默认
            if chamber_name == "PM1/2":
                proc_time = 300
            
            # 根据滞留时间判断颜色
            if max_time > proc_time + 15:
                value_color = self.theme.danger
            elif max_time > proc_time:
                value_color = self.theme.warning
            else:
                value_color = self.theme.success if count > 0 else self.theme.text_muted
            
            # 腔室行
            row_height = 18
            name_surf = self.font_tiny.render(chamber_name, True, self.theme.text_secondary)
            self.screen.blit(name_surf, (panel_x + 14, y))
            
            if count > 0:
                value_text = f"sum={(avg_time-proc_time)*count:.0f}|max={max_time-proc_time}s"
            else:
                value_text = "-|-"
            value_surf = self.font_mono.render(value_text, True, value_color)
            value_rect = value_surf.get_rect(right=panel_x + panel_width - 12, centery=y + row_height // 2)
            self.screen.blit(value_surf, value_rect)
        ...
        # ===== 运输（机械手）滞留时间 =====
        transport_label = self.font_tiny.render("TRANSPORT", True, self.theme.text_muted)
        self.screen.blit(transport_label, (panel_x + 12, y))
        y += 16
        
        transport_data = stats.get("transports", {"avg": 0, "max": 0, "count": 0})
        t_avg = transport_data.get("avg", 0)
        t_max = transport_data.get("max", 0)
        t_count = transport_data.get("count", 0)
        
        # 运输时间阈值（正常运输约 5s）
        if t_max > 10:
            t_color = self.theme.danger
        elif t_max > 7:
            t_color = self.theme.warning
        else:
            t_color = self.theme.success if t_count > 0 else self.theme.text_muted
        
        row_height = 18
        robot_label = self.font_tiny.render("ALL", True, self.theme.text_secondary)
        self.screen.blit(robot_label, (panel_x + 14, y))
        
        if t_count > 0:
            t_value_text = f"sum={(t_avg-5)*t_count:.1f}|max={t_max-5}s"
        else:
            t_value_text = "-|-"
        t_value_surf = self.font_mono.render(t_value_text, True, t_color)
        t_value_rect = t_value_surf.get_rect(right=panel_x + panel_width - 12, centery=y + row_height // 2)
        self.screen.blit(t_value_surf, t_value_rect)
    ...
    def _draw_right_panel(self):
        ...
        # Scale 按钮标签
        scale_y = self.scale_buttons[0].rect.top - 18
        scale_label = self.font_tiny.render("SCALE", True, self.theme.text_secondary)
        self.screen.blit(scale_label, (panel_x + 12, scale_y))
        
        # Scale 按钮
        for scale_btn in self.scale_buttons:
            # 高亮当前激活的缩放比例
            if abs(scale_btn.scale_value - self.scale_factor) < 0.01:
                # 当前激活的按钮用特殊颜色
                original_enabled = scale_btn.enabled
                scale_btn.enabled = True
                # 临时改变样式
                old_bg = self.theme.bg_surface
                self.theme.bg_surface = self.theme.dim_color(self.theme.accent, 0.5)
                scale_btn.draw(self.screen, self.font_tiny)
                self.theme.bg_surface = old_bg
                scale_btn.enabled = original_enabled
            else:
                scale_btn.draw(self.screen, self.font_tiny)
        ...
        hint_surf = self.font_tiny.render("Keys: [1-6] [W] [R] [M] [A]", True, self.theme.text_muted)
    ...
    def _draw_robot_buffer(self, wafer_info: Dict[str, List[Dict]]):
        """绘制双机械手缓冲区（TM2 和 TM3）"""
        # 根据路线设计分配运输库所到机械手
        # TM2 负责：LP->s1, s1->s2, s4->s5, s5->LP_done
        # TM3 负责：s2->s3, s3->s4
        tm2_transports = ["d_s1", "d_s2", "d_s5", "d_LP_done"]
        tm3_transports = ["d_s3", "d_s4"]
        
        # 收集 TM2 运输中的晶圆
        tm2_wafers = []
        for t_name in tm2_transports:
            tm2_wafers.extend(wafer_info.get(t_name, []))
        
        # 收集 TM3 运输中的晶圆
        tm3_wafers = []
        for t_name in tm3_transports:
    ...
    def _draw_robot_connections(self, target_chambers: set = None):
        ...
        for chamber_name, pos in self.chamber_positions.items():
            ...
            robot_names = config.get("robot", "TM2")
            is_target = chamber_name in target_chambers
            
            if isinstance(robot_names, (list, tuple, set)):
            ...
            for robot_name in robots:
                ...
                # 计算折线路径
                points = self._calc_polyline_path(
                    cx, cy, robot_size, chamber_cx, chamber_cy, chamber_name, robot_name
                )
                
                # 选择颜色和样式（增强可见性）
                if is_target:
                    # 目标腔室：脉冲发光效果 + 更粗的线条
                    pulse = self.anim.pulse(frequency=1.5, min_val=0.8, max_val=1.2)
                    line_color = self.theme.brighten_color(self.theme.accent, pulse)
                    line_width = 4
                    # 绘制发光效果（先画粗的半透明线）
                    glow_color = (*line_color, 100)
                    self._draw_polyline_with_glow(points, line_color, line_width, glow_color, 8)
                elif is_active:
                    # 普通活跃腔室：更亮的实线
                    line_color = self.theme.brighten_color(self.theme.accent, 0.85)
                    line_width = 3
                    self._draw_polyline(points, line_color, line_width)
                else:
                    # 闲置腔室：更明显的虚线
                    line_color = self.theme.dim_color(self.theme.border, 0.6)
                    line_width = 2
                    self._draw_dashed_polyline(points, line_color, line_width, 6, 4)
    
    def _calc_polyline_path(self, cx: int, cy: int, robot_size: int,
                            target_x: int, target_y: int, chamber_name: str,
                            robot_name: str) -> List[Tuple[int, int]]:
    ...
    def _draw_polyline_with_glow(self, points: List[Tuple[int, int]], color: Tuple[int, int, int], 
                                  width: int, glow_color: Tuple[int, int, int, int], glow_width: int):
        """绘制带发光效果的折线"""
        if len(points) < 2:
            return
        ...
        for i in range(len(points) - 1):
            pygame.draw.line(glow_surface, glow_color, points[i], points[i + 1], glow_width)
    ...
    def _draw_vertical_text(self, text: str, x: int, y: int, font: pygame.font.Font, 
                             color: Tuple[int, int, int], align: str = "center"):
        """绘制竖排文字
        ...
        Args:
            ...
            font: 字体
            color: 颜色
            align: 对齐方式 "left"（文字在x右侧）或 "right"（文字在x左侧）
        """
    ...
    def _is_chamber_on_left(self, x: int) -> bool:
        """判断腔室是否在中轴线左侧"""
        center_x = self.center_pos[0]
        chamber_center = x + self.CHAMBER_SIZE // 2
        return chamber_center < center_x
    ...
    def _draw_idle_chamber(self, name: str, x: int, y: int):
        ...
        # 名称（竖排，根据位置在左侧或右侧）
        is_left = self._is_chamber_on_left(x)
        if is_left:
            # 左侧腔室：名称在腔室左边
            name_x = x - 8
            self._draw_vertical_text(name, name_x, y + size // 2, self.font_small, 
                                    self.theme.text_muted, align="right")
        else:
            # 右侧腔室：名称在腔室右边
            name_x = x + size + 8
            self._draw_vertical_text(name, name_x, y + size // 2, self.font_small,
                                    self.theme.text_muted, align="left")
        
        # IDLE 标签（腔室内部中心）
        idle_surf = self.font_tiny.render("IDLE", True, self.theme.text_muted)
        idle_rect = idle_surf.get_rect(centerx=x + size // 2, centery=y + size // 2)
    ...
    def _draw_active_chamber(self, name: str, x: int, y: int, proc_time: int, 
        ...
        if is_left:
            ...
            name_x = x - 8
            self._draw_vertical_text(name, name_x, y + size // 2, self.font_small,
                                    self.theme.text_primary, align="right")
        else:
            # 右侧腔室：名称在腔室右边
            name_x = x + size + 8
            self._draw_vertical_text(name, name_x, y + size // 2, self.font_small,
                                    self.theme.text_primary, align="left")
    ...
    def _draw_process_info_panel(self, center_x: int):
        """绘制工艺信息面板 - 显示加工路线、晶圆数、约束信息"""
        # 面板位置和尺寸（增大以适应更长的路线）
        panel_width = 700
        panel_height = 55
        ...
        pygame.draw.rect(self.screen, self.theme.bg_deep, panel_rect, border_radius=8)
        pygame.draw.rect(self.screen, self.theme.border, panel_rect, 1, border_radius=8)
        ...
        # ===== 第一行：加工路线 =====
        route_label = self.font_tiny.render("ROUTE:", True, self.theme.text_muted)
        self.screen.blit(route_label, (inner_x, inner_y))
        
        # 绘制路线箭头图示 - 新六步路线
        # LP -> PM7/8(s1) -> LLC(s2) -> PM1/2(s3) -> LLD(s4) -> PM9/10(s5) -> LP_done
        route_x = inner_x + 50
        route_modules = ["LLA", "PM7/8", "LLC", "PM1/2", "LLD", "PM9/10", "LLB"]
        for i, mod in enumerate(route_modules):
            # 模块名称（根据是否为加工腔室设置颜色）
            if mod.startswith("PM"):
                mod_color = self.theme.accent
            elif mod in ["LLC", "LLD"]:
                mod_color = self.theme.info  # 交接点用信息蓝色
            else:
                mod_color = self.theme.text_secondary
            mod_surf = self.font_tiny.render(mod, True, mod_color)
            self.screen.blit(mod_surf, (route_x, inner_y))
            route_x += mod_surf.get_width() + 3
            
            # 箭头
            if i < len(route_modules) - 1:
                arrow_surf = self.font_tiny.render("→", True, self.theme.text_muted)
                self.screen.blit(arrow_surf, (route_x, inner_y))
                route_x += arrow_surf.get_width() + 3
        
        # ===== 分隔线 =====
        sep_x = route_x + 10
        pygame.draw.line(self.screen, self.theme.border_muted,
                        (sep_x, panel_y + 6), (sep_x, panel_y + panel_height - 6), 1)
        
        # ===== 晶圆数量 =====
        info_x = sep_x + 10
        wafer_label = self.font_tiny.render("Wafers:", True, self.theme.text_muted)
        self.screen.blit(wafer_label, (info_x, inner_y))
        wafer_val = self.font_tiny.render(str(self.net.n_wafer), True, self.theme.success)
        self.screen.blit(wafer_val, (info_x + wafer_label.get_width() + 3, inner_y))

        # ===== 机械手容量 =====
        arm_label = self.font_tiny.render("TM2/3 arms:1", True, self.theme.text_muted)
        self.screen.blit(arm_label, (info_x + wafer_label.get_width() + 36, inner_y))

        # ===== 第二行：约束信息 =====
        inner_y2 = inner_y + 20
        
        # 获取约束参数 - 从新的腔室名称获取
        s1_ptime = 70   # PM7/8 默认值
        s3_ptime = 300  # PM1/2 默认值
        s5_ptime = 70   # PM9/10 默认值
        p_res_time = getattr(self.net, 'P_Residual_time', 15)
        
        # 从配置获取实际处理时间
        for place in self.net.marks:
            if place.name == "s1":
                s1_ptime = place.processing_time
            elif place.name == "s3":
                s3_ptime = place.processing_time
            elif place.name == "s5":
                s5_ptime = place.processing_time
        
        # 约束信息
        constraints = [
            (f"PM7/8: {s1_ptime}s", self.theme.accent),
            (f"PM1/2: {s3_ptime}s", self.theme.accent),
            (f"PM9/10: {s5_ptime}s", self.theme.accent),
        ]
        
        constraint_x = inner_x
        for text, color in constraints:
            text_surf = self.font_tiny.render(text, True, color)
            self.screen.blit(text_surf, (constraint_x, inner_y2))
            constraint_x += text_surf.get_width() + 20
        
        # 时间和步数
        time_text = f"Residual Time<=20s | Dilivery Time<=10s"
        #if self.done:
        #    time_text += "  |  ENDED"
        time_color = self.theme.warning
        time_surf = self.font_tiny.render(time_text, True, time_color)
        time_rect = time_surf.get_rect(right=panel_x + panel_width - 6, centery=inner_y2 + 7)
    ...
    def draw(self):
        ...
        title_rect = title_surf.get_rect(centerx=center_x, top=10)
        self.screen.blit(title_surf, title_rect)
        
        # ========== 绘制工艺信息面板 ==========
        self._draw_process_info_panel(center_x)
    ...
    def handle_events(self) -> Optional[int]:
        ...
        for scale_btn in self.scale_buttons:
            scale_btn.check_hover(mouse_pos)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key in self.shortcuts:
                    ...
                    if action_id == -2:  # Reset 始终可用
                        ...
                        return -1
                    elif action_id == -3:  # Model(one step)
                        if self.policy is not None:
                            return -3
                    elif action_id == -4:  # Model(auto) - 切换按钮
                        if self.policy is not None:
                            return -4
                    elif action_id >= 0:
                        # 检查变迁/WAIT 是否可用
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    ...
                    # 检查缩放按钮点击
                    for scale_btn in self.scale_buttons:
                        if scale_btn.check_click(mouse_pos):
                            self.set_scale(scale_btn.scale_value)
                            return None
    ...
    def reset(self):
        ...
        # 停止自动执行
        self.auto_model_running = False
        self.auto_model_button.is_active = False
    ...
    def set_scale(self, scale: float):
        ...
        self.AUTO_STEP_INTERVAL = 0.2 / scale  # 根据缩放调整自动步进间隔
    ...
    def run(self):
        ...
        while self.running:
            ...
            # 处理自动模型执行
            if self.auto_model_running and not self.done:
            ...
            if action is not None:
                if action == -2:
                    ...
                    # Model(auto) 切换
                    self.auto_model_running = not self.auto_model_running
                    self.auto_model_button.is_active = self.auto_model_running
                    if self.auto_model_running:
                        self.last_auto_step_time = pytime.time()
                else:
                    self._execute_action(action)
...
def main():
    ...
    if not args.no_model:
        if args.model:
            ...
            default_model = os.path.join(current_dir, "..", "PPO", "saved_models","20260123_231624", "CT_phase2_best.pt")
